...
func (s SimpleChat) HandleMediaMsg(msg *message.MixMessage) string {
    ...
    case message.MsgTypeEvent:
        if msg.Event == message.EventSubscribe {
            ...
            default:
                return fmt.Sprintf("unknown event key=%v", msg.EventKey)
            }
        } else {
            return "不支持的类型"
        }
...
func SetModel(param, userId, modelName string) string {
    botType := config.GetUserBotType(userId)
    if botType == config.Bot_Type_Gpt || botType == config.Bot_Type_Gemini || botType == config.Bot_Type_Qwen {
        if err := db.SetModel(userId, botType, modelName); err != nil {
            return fmt.Sprintf("%s 设置model失败", botType)
        }
        return fmt.Sprintf("%s 设置model成功，当前model: %s", botType, modelName)
    }
